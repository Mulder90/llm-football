...
2. **Playing time:** 3,600 eligible ticks per half, defined once in the current ruleset (football-0.9). Restart setup/ready and halftime pause this clock. Ends swap after the interval. The final match uses one minute per half, two minutes total; bounded excerpts support shorter development tests.

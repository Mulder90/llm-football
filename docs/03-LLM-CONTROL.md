...
- `ball`: position, velocity, owner, last touch, `possessionMode` (`loose`, `feet`, `hands`) nullable `holdTicksRemaining`, and signed `pitchClearanceMetres` to the left/right/top/bottom lines; `offside`: the public current snapshot.
...
Goal geometry is measured from the exact ball position toward the current opponent goal, then rounded to two decimals. The opening is the angle between the two post vectors; it ignores defenders, the keeper and ball height, so it is not a scoring probability. A wide carrier can get closer to the goal line while losing angle. `atan2(abs(cross), dot)` stays finite even at a post (defined as 0 there); between the posts on the goal plane it is 180 degrees. Pitch clearance uses the ball centre and can be negative; it does not predict whole-ball out or choose a destination.

The prompt begins with the objective to play football, score more than the opponent and win. Carrying, passing and possession serve chance creation and defence. It asks for good shots rather than waiting for perfection, cutbacks/passes/recycling at narrow angles, clearance for the foot offset, and factual revisions when a carry stalls or worsens. These are model instructions, not automatic shots, timed-release rules or engine-selected targets.
...
One stable JSON schema is used across requests and recorded in provenance. The provider schema omits only `batch.orders.maxItems`: Gemini rejects that cap on the expanded action-union array. The local Zod schema still rejects more than eleven orders before commitment, and memory array caps remain in the provider schema. Identity values remain in `responseIdentity` and must match the frozen state exactly in runtime validation; unsupported versions, stale replies and wrong-team orders remain rejected. Compact JSON and brief public notes reduce repeated output. See [decision 010](decisions/010-CARRIER-CHOICES-AND-REQUESTS.md).

`teamContext.keeperId` names the team's active keeper, or null after dismissal. The prompt explicitly reserves guard/pickup/distribute/put_down for that player. Outfield defending, marking and covering use model-chosen move/hold orders; guard already moves the keeper and has no pace field. This exposes existing role restrictions without adding provider-schema enums or automatic tactics.

Rejected recognized actions report the offending field, such as `batch.orders.1.pace`, rather than the action union's generic “Invalid input.” Outfield keeper-action errors identify the action and player and explain the legal positioning forms. The original validator still rejects the entire batch; feedback neither inserts a missing pace nor converts guard into move. Repairs remain bounded and use the same frozen observation as the initial request.

## Evaluation and replay

The sustained-play harness reuses the actual paired match loop from fresh scenario states. Both controllers can replan through a bounded playing-time horizon. An optional `evaluation` observation contains `plannedPlayingTicks` and `remainingPlayingTicks`; `secondsUntilNextScheduledDecision` still describes the one-second cadence, and regular match clocks stay intact. Scripted traces use the same schema, memory, repairs and receipts with explicit scripted provenance. The CLI defaults to scripted mode; explicit model mode runs the reviewed pair under a shared trial allowance. [Slice 19](slices/19-SUSTAINED-PLAY-HARNESS.md) records the baseline.

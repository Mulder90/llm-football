...
Use very quiet real stadium ambience, ball/contact sounds, a distinct goal roar and recognizable referee signals. The user rejected the synthesized drum sound, so all percussion is removed, including short save/miss and landing accents. Filtered-noise ambience, harmonic synthetic crowd voices and procedural claps remain absent. The approved whistle is unchanged. Use original synthesis or properly licensed assets. No inference is needed for playback audio.

Trigger effects from stable events crossed by the forward playhead. Scrubbing must not replay all historical goals/whistles; reset event handling on seek. The crowd loop plays only at 1×, stays very low and stops on pause, seek, mute, hidden tabs and full time. Sound defaults to enabled and starts with the user's play gesture; a prior mute choice is respected. Clock-transition whistles align with their recorded frame, including the full-time endpoint; physical incident cues follow the completed step. The final whistle may finish after automatic playback completion. Crowd reactions cannot reveal future outcomes.

Attacking buildup remains visible through the supporters and robots. A 30-second, overlapped real stadium loop supplies subtle background life, while the existing licensed goal roar retains supporter-side placement. Source, edits and the distinction between numerical audio checks and listening feedback are documented in [sound credits](SOUND-CREDITS.md).

...
Open the local URL printed by Vite; the port depends on what's available. Press **Watch the match** to start the unfinished model passing excerpt. **Inside the match → Matches** also offers **Keeper buildup · Cyan fallback** and the scripted **Safe hands, open play** demo. Watching and replaying require no API keys or provider requests.

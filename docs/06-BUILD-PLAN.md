...
The funded one-minute local match is complete and available as **One minute, two models**. It improves carrying and provider reliability, but exposes a keeper catch-position defect that affects the scoring sequence. Resolve that defect before the conditional final version with two minutes per half and the publication pass. The local build is the readiness reference; leave the older live website alone until the new version is ready. [Current progress](PROGRESS.md) records measured results; [historical handoffs](slices/README.md) preserve earlier work.

...
The entire approved trial, including six diagnostic requests and both continuations, used **61 requests, 393.075 generation seconds and $0.43614375 estimated**: $0.10788 OpenAI / $0.32826375 Gemini. Failed requests without usage retain conservative reservations; this is not confirmed billing. The remaining Gemini allowance is $0.07173625, below the $0.079872 reservation needed for another paired boundary. No generation is running and no allowance resets on a new CLI invocation. The full run is deferred pending the user’s review and additional credit. [Slice 21](slices/21-MODEL-SEQUENCES-AND-SCHEMA.md) records the full ledger and results; [slice 20](slices/20-PAIRED-MODEL-TRIAL.md) retains the original failure.
